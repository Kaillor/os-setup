...
Usage: $(basename "${BASH_SOURCE[0]}") <path>
  path        Path that corresponds to a file containing a list of absolute
              file paths that need to be patched, or a directory. If the path
              ...
              paths that need to be patched. Corresponding patch files must be
              placed next to each file processed this way in a directory called
