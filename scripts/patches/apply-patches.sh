...
main() {
  ...
  local list_of_files_to_patch="$1"

  if file_exists "$list_of_files_to_patch"; then
    apply_patches "$(realpath "$list_of_files_to_patch")"
  elif directory_exists "$list_of_files_to_patch"; then
    find "$list_of_files_to_patch" -name "$FILES_TO_PATCH_NAME" -print0 | xargs -0 -I {} apply_patches "$(realpath "{}")"
  else
    error "Path '$list_of_files_to_patch' does not exist."
    return 1
  fi
